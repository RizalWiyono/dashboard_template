...
define('APIKEY', '812DYeZSHAQkD7UQQeN0bVd3CPJKu87u');

include '../../../src/connection/connection.php';
    $query = "SELECT date FROM `tb_sales` WHERE market_id=1 GROUP BY date DESC LIMIT 1";
  ...
  $i=1;
    ...
    while ($i <= $num_page) {
      ...
      foreach ($results_per_page["results"] as $item) {
        ...
        $sql_value_exist = mysqli_query($connect, "SELECT * FROM tb_sales WHERE market_id=1 && extra='$extra'");
        if(mysqli_num_rows($sql_value_exist) < 1){
          ...
          VALUES 
          (NULL, 1, '$item_id', '$order_id', '$year', '$month', '$day', '$date_1', '$type', '$earnings', '$county', '$extra')");
...
foreach ($results_upd["new-files-from-user"] as $item) {
  ...
  if(mysqli_num_rows($sql_value_exist) < 1)
  {
      $sql_insert_type = mysqli_query($connect, "INSERT INTO tb_catalog (catalog_id , item_id, market_id, upload_on, category, sub_category, color, slug, name) 
      VALUES 
      (NULL, '$item_id', 1, '$date_cd', '', '' , '' , '', '$detail')");

      mysqli_query($connect, "UPDATE tb_catalog SET name = REPLACE(name, '(Regular License)', '') WHERE market_id=1");
      mysqli_query($connect, "UPDATE tb_catalog SET upload_on = '$date_cd' WHERE market_id=1 && item_id='$item_id'");
      mysqli_query($connect, "UPDATE tb_catalog SET name = REPLACE(name, '(Regular License)', '')");

      mysqli_query($connect, "UPDATE tb_catalog SET slug = 'PowerPoint' WHERE name LIKE '%powerpoint%' && market_id=1");
      mysqli_query($connect, "UPDATE tb_catalog SET slug = 'Keynote' WHERE name LIKE '%keynote%' && market_id=1");
      mysqli_query($connect, "UPDATE tb_catalog SET slug = 'Potrait' WHERE name LIKE '%potrait%' && market_id=1");
      mysqli_query($connect, "UPDATE tb_catalog SET slug = 'Google Slide' WHERE name LIKE '%google slide%' && market_id=1");
      mysqli_query($connect, "UPDATE tb_catalog SET slug = 'Etc' WHERE slug='' && market_id=1");
  }
